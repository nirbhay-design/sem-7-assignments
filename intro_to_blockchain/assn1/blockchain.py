...
class wallet:
    ...
    def sign_transaction(self,transaction:dict):
        # for to
        is_transaction_possible = self.utxo.is_transaction_possible(transaction['btc'])
        ...
        signaturized_transaction = {**transaction,"sign":transaction_signature}
...
class block:
    def __init__(self,**kwargs):
        ...
        self.nonce = None
        self.cur_hash = None
        self.transaction_hash = kwargs['transaction_hash']
    ...
    def mine_block(self,transaction_list: list):
        ...
        if self.blockchain:
            new_block = block(
                    ...
                    transaction_hash = transaction_list,
                    starting_nonce = 0
                )
        else:
            new_block = block(
...
class blockchain:
    def __init__(self,n_miners = 10, n_users=20):
        ...
        for i in range(n_users):
            self.user_list[i].w.add_utxo({"from":self.user_list[random.randint(0,n_users-1)].pkhash(),"to":self.user_list[i].pkhash(),'btc':30})
            self.user_list[i].w.add_utxo({"from":self.user_list[random.randint(0,n_users-1)].pkhash(),"to":self.user_list[i].pkhash(),'btc':30})
            self.user_list[i].w.add_utxo({"from":self.user_list[random.randint(0,n_users-1)].pkhash(),"to":self.user_list[i].pkhash(),'btc':30})
            self.user_list[i].w.add_utxo({"from":self.user_list[random.randint(0,n_users-1)].pkhash(),"to":self.user_list[i].pkhash(),'btc':20})

    def transaction(self, transactions):
        transactions_hash = []
        for from_idx, to_idx, btc, reward in transactions:
            transact = {"from":self.user_list[from_idx].pkhash(), "to":self.user_list[to_idx].pkhash(), "btc": btc}
            signed_transaction = self.user_list[from_idx].w.sign_transaction(transact)
            ...
            transaction_message = transact['from']+transact['to']+str(transact['btc'])
            transaction_hash = find_hash(transaction_message)
            transactions_hash.append(transaction_hash)

        # randomly select a miner to do the mining
        random_miner = random.randint(0, self.n_miners-1)
        self.miner_list[random_miner].mine_block(transactions_hash)
        self.miner_list[random_miner].pass_blockchain()

        for from_idx, to_idx, btc, reward in transactions:
            transact = {"from":self.user_list[from_idx].pkhash(), "to":self.user_list[to_idx].pkhash(), "btc": btc}
            self.user_list[from_idx].w.update_utxo(transact['btc'])
    ...
    while 1:
        ...
        if choice == 0:
            ...
            for trans in range(n_transactions):
                tran_info = list(map(int, input().split()))
